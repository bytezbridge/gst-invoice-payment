...
export function welcomeEmail({ email, license_key }) {
  ...
  const bodyHtml = `
    ...
    <p style="margin:0 0 8px 0;color:#334155;"><strong>Open Claude Code in your Terminal app</strong> (download free from <a href="https://claude.ai/code" style="color:#4F46E5;">claude.ai/code</a> if not installed). Once Claude Code is running, type these two commands at its prompt &mdash; these are slash commands inside Claude Code, NOT shell commands:</p>
    <div style="font-size:11px;font-weight:700;color:#64748B;letter-spacing:.6px;text-transform:uppercase;margin:0 0 6px 0;">Type at the Claude Code prompt:</div>
...
${esc(ccCmd2)}</pre>
    ...
    <p style="margin:0 0 8px 0;color:#334155;"><strong>Open the Claude Cowork desktop app</strong> (download free from <a href="https://claude.ai/cowork" style="color:#4F46E5;">claude.ai/cowork</a> if not installed).</p>
    ...
    <h2 style="margin:20px 0 8px 0;font-size:16px;color:#4F46E5;">Works on Free Claude too</h2>
    <p style="margin:0 0 14px 0;color:#334155;">You don&#39;t need Claude Pro or Max. The free Claude tier works fine. Plugins are available on every Claude tier.</p>
